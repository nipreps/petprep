...
def init_single_subject_wf(subject_id: str):
    ...
    if not anat_only and not subject_data['pet']:
        raise RuntimeError(
            "No PET data found for participant {}. "
            "All workflows require PET images.".format(
                subject_id if not subject_id else '<all>'
            )
        )
...
""".format(
        petprep_ver=config.environment.version, nipype_ver=config.environment.nipype_version
    )
...
: For each of the {num_pet} PET data found per subject (across all
tasks and sessions), the following preprocessing was performed.
""".format(
        num_pet=len(subject_data['pet'])
    )
