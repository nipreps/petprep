"""Tests for :mod:`petprep.interfaces.bids`."""
...
def test_BIDSURI():
    ...
    interface.inputs.in1 = '/data/sub-01/pet/sub-01_pet.nii.gz'
    results = interface.run()
    assert results.outputs.out == ['bids:raw:sub-01/pet/sub-01_pet.nii.gz']
    ...
    interface.inputs.in2 = [
        '/data/derivatives/source-1/sub-01/pet/sub-01_pet.nii.gz',
    ...
    assert results.outputs.out == [
        ...
        '/out/sub-01/pet/sub-01_pet.nii.gz',  # No change
    ]
    ...
    interface.inputs.in1 = [
        '/data/sub-01/pet/sub-01_pet.nii.gz',
        'bids:raw:sub-01/pet/sub-01_petref.nii.gz',
    ]
    interface.inputs.in2 = [
        '/data/derivatives/source-1/sub-01/pet/sub-01_pet.nii.gz',
        '/out/sub-01/pet/sub-01_pet.nii.gz',
    ...
    assert results.outputs.out == [
        'bids:raw:sub-01/pet/sub-01_pet.nii.gz',
        'bids:raw:sub-01/pet/sub-01_petref.nii.gz',  # No change
        'bids:deriv-0:sub-01/pet/sub-01_pet.nii.gz',
        '/out/sub-01/pet/sub-01_pet.nii.gz',  # No change
    ]
