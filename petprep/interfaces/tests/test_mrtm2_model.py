...
from petprep.interfaces.kinmod import MRTM2

def test_mrtm2_model():
    ...
    mrtm2_node = Node(MRTM2(), name="mrtm2_model")
    mrtm2_node.inputs.reference_tac = reference_tac
    mrtm2_node.inputs.target_tac = target_tac
    mrtm2_node.inputs.frame_times = frame_times
    mrtm2_node.inputs.k2p = k2p
    mrtm2_node.inputs.frame_weight = frame_weight

    wf = Workflow(name="mrtm2_wf")
    wf.add_nodes([mrtm2_node])

    execgraph = wf.run()

    results = None
    for node in execgraph.nodes():
        if node.name == "mrtm2_model":
