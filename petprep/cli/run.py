...
def main():
    ...
    config.loggers.workflow.log(
        15,
        "\n".join(["PETPrep config:"] + ["\t\t%s" % s for s in config.dumps().splitlines()]),
    )
    ...
    try:
        ...
        if config.workflow.run_reconall:
            ...
            _copy_any(dseg_tsv, str(config.execution.peprep_dir / "desc-aparcaseg_dseg.tsv"))
        errno = 0
    finally:
        ...
        write_derivative_description(config.execution.bids_dir, config.execution.petprep_dir)
